...
## Example
The following example generates 3 continuous, 1 binary, 1 ordinal, 3 Poisson, and 2 Negative Binomial variables.  The standardized cumulants produce power method constants that yield valid pdfs, so no sixth cumulant corrections are needed.  See the **Using the Sixth Cumulant Correction to Find Valid Power Method Pdfs** vignette for examples of using the sixth cumulant correction.  (Note that the `printr` package is invoked to display the tables.)
